...
<div class="no-print" style="max-width: 820px; margin: 0 auto 1.25rem auto; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 0.75rem;">
  ...
  <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
    <?php if (!empty($waUrl)): ?>
      <a href="<?= $waUrl ?>" target="_blank" class="btn" style="background: #25d366; color: #ffffff; font-size: 0.85rem; font-weight: 700; display: inline-flex; align-items: center; gap: 0.4rem;" title="Compartir cotización por WhatsApp">
        <i class="fa-brands fa-whatsapp" style="font-size: 1rem;"></i> Enviar por WhatsApp
      </a>
    <?php endif; ?>
    <button onclick="guardarPDF()" class="btn btn-primary" style="font-size: 0.85rem; font-weight: 700; display: inline-flex; align-items: center; gap: 0.4rem;">
      <i class="fa-solid fa-file-pdf"></i> Imprimir / Guardar en PDF
    </button>
  </div>
</div>

<!-- Barra de Ayuda para Enviar PDF por WhatsApp -->
<div class="no-print" style="max-width: 820px; margin: 0 auto 1.25rem auto; background: rgba(56, 189, 248, 0.08); border: 1px solid rgba(56, 189, 248, 0.25); border-radius: 8px; padding: 0.65rem 1rem; font-size: 0.82rem; color: #93c5fd; display: flex; align-items: center; gap: 0.65rem;">
  <i class="fa-solid fa-circle-info" style="font-size: 1.1rem; color: #38bdf8; flex-shrink: 0;"></i>
  <div>
    <strong>¿Cómo enviar este PDF por WhatsApp al cliente?</strong><br>
    <span><strong>1.</strong> Presiona <span style="color: #fff; font-weight: 600;">[Imprimir / Guardar en PDF]</span> y selecciona <em>"Guardar como PDF"</em> (se guardará como <code>Cotizacion_<?= $folio ?>_<?= htmlspecialchars($ot['Patente']) ?>.pdf</code>).<br>
    <strong>2.</strong> Presiona <span style="color: #25d366; font-weight: 600;">[Enviar por WhatsApp]</span> para abrir el chat del cliente.<br>
    <strong>3.</strong> Arrastra el archivo PDF descargado a la ventana del chat (o haz clic en el icono del clip 📎 &gt; Documento).</span>
  </div>
</div>
...
function guardarPDF() {
  ...
  document.title = 'Cotizacion_<?= $folio ?>_<?= htmlspecialchars($ot['Patente']) ?>';
  window.print();
...
<div class="doc-sheet">
  ...
  <!-- Resumen de Motivo de Ingreso y Diagnóstico Inicial -->
  <?php if (!empty($ot['OperacionesTextoLibre']) || !empty($operacionesSolicitadas) || !empty($hallazgos)): ?>
    <div style="background: #f1f5f9; border-left: 4px solid #2563eb; border-radius: 4px; padding: 0.75rem 1rem; margin-bottom: 1.25rem; font-size: 0.82rem;">
      <div style="font-weight: 800; color: #1e3a8a; text-transform: uppercase; font-size: 0.72rem; margin-bottom: 0.35rem; letter-spacing: 0.05em;">
        <i class="fa-solid fa-clipboard-check"></i> Motivo de Ingreso y Diagnóstico del Taller
      </div>
      <?php if (!empty($ot['OperacionesTextoLibre'])): ?>
        <div style="margin-bottom: 0.3rem;">
          <strong>Petición del cliente:</strong> <em>"<?= htmlspecialchars($ot['OperacionesTextoLibre']) ?>"</em>
        </div>
      <?php endif; ?>
      <?php if (!empty($operacionesSolicitadas)): ?>
        <div style="margin-bottom: 0.3rem;">
          <strong>Trabajos solicitados en recepción:</strong>
          <?= implode(', ', array_map(fn($o) => htmlspecialchars($o['NombreOperacion']), $operacionesSolicitadas)) ?>
        </div>
      <?php endif; ?>
      <?php if (!empty($hallazgos)): ?>
        <div>
          <strong>Diagnóstico técnico:</strong>
          <?= implode(' · ', array_map(fn($h) => htmlspecialchars($h['Area'] . ': ' . $h['Hallazgo']), $hallazgos)) ?>
        </div>
      <?php endif; ?>
    </div>
  <?php endif; ?>
        ...
        <?php foreach ($lineasRepuesto as $lr): ?>
          <tr>
            <td>
              ...
              <?php if (!empty($lr['Origen']) && $lr['Origen'] !== 'Directo'): ?>
                <span style="font-size: 0.68rem; background: #e0f2fe; color: #0369a1; padding: 1px 5px; border-radius: 3px; font-weight: 600; margin-left: 4px;">
                  <?= $lr['Origen'] === 'Cliente' ? 'Solicitud Cliente' : ($lr['Origen'] === 'Diagnostico' ? 'Diagnóstico' : 'Fluidos') ?>
                </span>
              <?php endif; ?>
        ...
        <?php foreach ($lineasManoObra as $lm): ?>
          <tr>
            <td>
              ...
              <?php if (!empty($lm['Origen']) && $lm['Origen'] !== 'Directo'): ?>
                <span style="font-size: 0.68rem; background: #fef3c7; color: #92400e; padding: 1px 5px; border-radius: 3px; font-weight: 600; margin-left: 4px;">
                  <?= $lm['Origen'] === 'Cliente' ? 'Solicitud Cliente' : ($lm['Origen'] === 'Diagnostico' ? 'Diagnóstico' : 'Fluidos') ?>
                </span>
              <?php endif; ?>
        ...
        <?php foreach ($lineasTerceros as $lt): ?>
          <tr>
            <td>
              ...
              <?php if (!empty($lt['Origen']) && $lt['Origen'] !== 'Directo'): ?>
                <span style="font-size: 0.68rem; background: #e2e8f0; color: #334155; padding: 1px 5px; border-radius: 3px; font-weight: 600; margin-left: 4px;">
                  <?= $lt['Origen'] === 'Cliente' ? 'Solicitud Cliente' : ($lt['Origen'] === 'Diagnostico' ? 'Diagnóstico' : 'Fluidos') ?>
                </span>
              <?php endif; ?>
