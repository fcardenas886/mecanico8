...
<div class="no-print" style="display: flex; justify-content: space-between; align-items: center; gap: 1rem; flex-wrap: wrap; margin-bottom: 1rem;">
  <div>
    ...
    <p style="color: var(--text-muted); font-size: 0.9rem;">
      <strong><?= htmlspecialchars($folio) ?></strong> · <?= htmlspecialchars($ot['Patente']) ?> <?= htmlspecialchars($ot['Marca']) ?> <?= htmlspecialchars($ot['Modelo']) ?> — <strong style="color: #fff;"><?= htmlspecialchars($ot['ClienteNombre']) ?></strong>
...
<div id="modalTelCliente" class="no-print" style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.75); backdrop-filter: blur(6px); z-index: 9999; align-items: center; justify-content: center;">
  <div style="background: var(--card-bg); border: 1px solid var(--border-dark); border-radius: 14px; width: 420px; padding: 1.75rem; box-shadow: 0 10px 25px rgba(0,0,0,0.5);">
    ...
    <p style="font-size: 0.84rem; color: var(--text-muted); margin-bottom: 1.25rem;">
      Ingresa el número de <strong><?= htmlspecialchars($ot['ClienteNombre']) ?></strong> para enviar el comprobante de recepción directo a su WhatsApp:
    </p>
    <form method="POST" action="comprobante_ot.php?id=<?= $otId ?>">
      <?= csrfField() ?>
