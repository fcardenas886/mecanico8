        ...
        if ($action === 'agregar_repuesto') {
            ...
            if (!empty($_POST['ajax'])) {
                ...
                if ($error) {
                    ...
                    $stmtLine = $pdo->prepare("SELECT Subtotal, Cantidad, PrecioUnitario FROM presupuestodetalle WHERE PresupuestoDetalleID = :id");
                    $stmtLine->execute([':id' => $lineaId]);
                    $lineData = $stmtLine->fetch();

                    $stmtL = $pdo->prepare("SELECT * FROM presupuestodetalle WHERE PresupuestoID = :pid");
                    $stmtL->execute([':pid' => $presupuesto['PresupuestoID']]);
                    $allLineas = $stmtL->fetchAll();
                    $tot = 0;
                    foreach ($allLineas as $l) {
                        if ($l['PoliticaCobro'] !== 'SoloSiNoAprueba') $tot += (int)$l['Subtotal'];
                    }

                    echo json_encode([
                        'ok' => true,
                        'linea_id' => $lineaId,
                        'subtotal' => (int)($lineData['Subtotal'] ?? 0),
                        'subtotal_fmt' => '$' . number_format((int)($lineData['Subtotal'] ?? 0), 0, ',', '.'),
                        'total' => $tot,
                        'total_fmt' => '$' . number_format($tot, 0, ',', '.')
                    ]);
                }
                exit;
            }
